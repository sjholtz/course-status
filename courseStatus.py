...
course_dates = [datetime(2026, 1, 14),
                datetime(2026, 5,  1)]

# Dates to exclude, like Holidays and Spring Break:
exclude_dates = [datetime(2026, 1, 19), # MLK day
                 ...
                 datetime(2026, 3, 13)] # Spring Break Friday
...
quiz_rule_set.rrule(rrule(freq=WEEKLY,
...
# finals week:
assignment_rule_set.rrule(rrule(freq=WEEKLY,
...
if not base_path.exists():
    print("ERROR: File storage path must be exist or be mounted!!!",
          file=sys.stderr)
    sys.exit(1)
...
while True:
    try:
        ...
        if 1 <= current_module <= number_of_modules:
            ...
            current_module = input(f"ERROR: Current module students are working in [{week_number}]? ")
    except ValueError:
        current_module = input(f"ERROR: Current module students are working in ({week_number})? ")
...
while True:
    ...
    elif input_str[0:2] in ['04', '06', '09', '11'] and input_str[3] == '3' and input_str[4] > '0':
        input_str = get_month_day_str(month_day_str,
                                      error=True)
...
as_of_date = datetime.strptime(month_day_str + "-" +
                               str(today_date.year),
                               "%m-%d-%Y")
...
for filename in base_path.iterdir():
    ...
    if grades_path_filename == "" and "Grades" in name and month_day_str in name and ext == ".csv":
        grades_path_filename = filename
        print(f"\tGrade data:              {grades_path_filename}")
        continue
    # Get the first (latest date) missing work CSV file:
    if missing_work_path_filename == "" and month_day_str in name and ext == ".csv" and str(name).startswith("missingAssignments"):
...
if grades_path_filename == "" or missing_work_path_filename == "":
    print("ERROR: Download grades and missing assignments files " +
    ...
    if choice != "Y" and choice != "y":
        print("Everything is not OK: Quitting... " +
...
try: # To open the missing work file:
    ...
    print(f"{e.strerror}: {missing_work_path_filename}",
          file=sys.stderr)
    print("\tGenerate a missing work report from Canvas",
          file=sys.stderr)
    print("\tand rename it something like: missingAssignments-01-19-2023.csv.",
          file=sys.stderr)
    print("\tso that it has the correct MM-DD (month-day) string in it.",
          file=sys.stderr)
    sys.exit(1)
...
try: # To open this file:
    ...
    print(f"{e.strerror}: {grades_path_filename}",
          file=sys.stderr)
    print("ERROR: No grades file exists.",
          file=sys.stderr)
    print("\tExport a gradebook from Canvas.",
          file=sys.stderr)
    sys.exit(1)
else:
    freader = csv.reader(f)
...
for number, quiz in enumerate(quiz_due_dates,
                              start=1):
...
for number, assignment in enumerate(assignment_due_dates, start=1):
    if today_date < assignment + too_late_offset:
        ...
        next_assignment_too_late_date = (assignment + too_late_offset).strftime(date_format)
        break
...
for number, assignment in enumerate(assignment_due_dates, start=1):
    if today_date < assignment + resubmission_deadline_offset:
        ...
        next_assignment_resubmit_too_late_date = (assignment + resubmission_deadline_offset).strftime(date_format)
        break
...
for student in grades_data:
    ...
    to_csv.append([course_prefix + course,                  # Course
                   ...
                   next_assignment_resubmit_too_late_date]) # ResubmitDeadLine
...
try: # To open the file for writing:
    ...
    print(f"{e.strerror}: {base_path / today_path_str}",
          file=sys.stderr)
    sys.exit(1)
else:
